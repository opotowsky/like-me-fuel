...
def main():
    ...
    ## for PCA code ##
    nucs = XY.columns[~XY.columns.isin(lbls+nonlbls)].tolist()
    ...
    pca.fit(X)
    X = pd.DataFrame(pca.transform(X), index=XY.index, columns=['comp1', 'comp2', 'comp3'])
    testX = pd.DataFrame(pca.transform(testX), index=test.index, columns=['comp1', 'comp2', 'comp3'])
